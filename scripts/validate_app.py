...
required_index_tokens = {
    ...
    "agenda pessoal Ligar visível imediatamente": 'data-calendar-unlock>Ligar</button>',
    "agenda pessoal começa bloqueada": 'PERSONAL_CALENDAR_STATE={envelope:null,data:null,loadedAt:0,locked:true',
    "emparelhamento da agenda por fragmento local": "function consumePersonalCalendarPairingHash()",
    "chave de emparelhamento removida do endereço": "history.replaceState(null,document.title,location.pathname+location.search)",
    "emparelhamento dinâmico por hashchange": 'window.addEventListener("hashchange"',
    ...
    "timeout no carregamento da agenda": "async function fetchPersonalCalendarEnvelope(url,timeoutMs=8000)",
    "ficheiro raw consultado antes do Pages": 'fetchPersonalCalendarEnvelope(PERSONAL_CALENDAR_RAW_URL+"?ts="+stamp,8000)',
